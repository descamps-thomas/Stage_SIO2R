...
try {
    ...
    if (!$resultat) {
        ...
        if (isset($_POST['submit'])) {
            ...
            $requete = $bdd->prepare("UPDATE Partiepeche SET nom = :nom, nombremax = :nombre, date = :date, etangid = :etang, typepecheid = :typepecheid, horaire = :horaire, etat = :etat WHERE id = :id");
            ...
            $requete->bindParam(':horaire', $horaire);
...
?>
        <form action="Modifpartie.php?id=<?php echo $id; ?>" method="POST">
            <table>
                    ...
                    <td>
                        <select class="w3-select" name="etang">
                            ...
                            foreach ($typesEtang as $etang) {
                                $selected = ($etang['id'] == $etangStocke) ? 'selected' : '';
                                echo '<option value="' . $etang['id'] . '" ' . $selected . '>' . $etang['nometang'] . '</option>';
                            }
                            ?>
                        </select>
                    </td>
                    ...
                    <td>
                        <select class="w3-select" name="typeP">
                            ...
                            foreach ($typesDePeche as $type) {
                                $selected = ($type['id'] == $typepecheidStocke) ? 'selected' : '';
                                echo '<option value="' . $type['id'] . '" ' . $selected . '>' . $type['nomtypep'] . '</option>';
                            }
                            ?>
                        </select>
                    </td>
                ...
                <tr>
                <td><label for="etat">Horaire</label></td>
                    <td>
                    <select required="" class="w3-select" name="horaire"> 
                    ...
                    <td>
                        <select class="w3-select" name="etat">
                            <option value="1" <?php echo ($etatStocke == 1) ? 'selected' : ''; ?>>Actif</option>
                            <option value="2" <?php echo ($etatStocke == 2) ? 'selected' : ''; ?>>Archivé</option>
                        </select>
                    </td>
                </tr>
